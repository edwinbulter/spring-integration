...
public class KafkaToDbFlowConfig {
    ...
    private static final Logger log = LoggerFactory.getLogger(KafkaToDbFlowConfig.class);
    private static final String ERROR_CHANNEL_NAME = "kafkaToDbErrorChannel";
    ...
    public MessageChannel kafkaToDbErrorChannel() {
        return new DirectChannel();
    }

    @Bean
    public IntegrationFlow kafkaToDbErrorLoggingFlow() {
        return IntegrationFlow.from(ERROR_CHANNEL_NAME)
                .handle((GenericHandler<ErrorMessage>) (errorMessage, headers) -> {
                    ...
                    Message<?> failedMessage = cause instanceof MessagingException messagingException
                            ...
                            : null;
                    log.error("kafka-to-db-app failed to process message: payload={}, headers={}",
